package com.example.app_with_argocd.service;
...
import com.example.app_with_argocd.repository.AttachmentFileRepository;
...
public class S3Service {
    ...
    public void uploadS3File(MultipartFile file) throws Exception {
        ...
        if(file == null) {
            System.out.println("파일 전달 안됨");
            throw new Exception("파일 전달 오류 발생");
        }

        // DB 저장
        String filePath = "C://CE//09.eks//" + DIR_NAME;
        ...
        AttachmentFile attachmentFile = AttachmentFile.builder()
                ...
                .build();
        System.out.println(attachmentFile.toString());

        Long fileNo = fileRepository.saveAndFlush(attachmentFile).getAttachmentFileNo();


        if (fileNo != null) {
            // C:/CE/97.data/s3_data에 파일 저장

            File uploadFile = new File(attachmentFile.getFilePath() + "//" + attachmentFile.getAttachmentFileName());
            file.transferTo(uploadFile);

            // S3 전송 및 저장 (putObject)
            // bucketName
            // key : bucket 내부에 Object가 저장되는 위치 + 파일(객체)명
            // file : 물리적인 파일 소스
